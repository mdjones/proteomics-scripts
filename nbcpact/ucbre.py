...
class PeptidesFromPeptideListBuilder:
    ...
    def generate_peptides(self):
        peptideDF = self.generate_peptides_df()
        peptides = peptideDF.apply(self.__init_peptide, axis=1)

        return peptides


class PeptidesFromPDReaderBuilder:
    """
        Works with a *.pdResult file to replicate the input you would get from PeptidesFromPeptideListBuilder

    """

    def __init__(self, pdReader, num_uniprot_ids=np.inf):
        self.__pdReader = pdReader
        self.__num_uniprot_ids = num_uniprot_ids
    ...
    def generate_peptides(self):
        ...
        # Read the target peptides table
        df = None #self.

        df = self.__pdReader.get_target_peptides()
